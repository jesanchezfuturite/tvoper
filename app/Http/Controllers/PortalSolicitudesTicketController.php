...
class PortalSolicitudesTicketController extends Controller
{
  ...
  public function enCarrito(Request $request){
    ...
    $ids = $this->ticket->whereIn('id',$body['ids'])->pluck("clave")->toArray();
    try{
      if($body["type"]=="en_carrito"){
        $solicitudTicket = $this->ticket->whereIn('id',$ids)->update(['en_carrito'=>$body['status']]);
        $count = $this->ticket->where("en_carrito", 1)->count();
        $mensaje="Solicitudes en el carrito";
      }

      if($body["type"]=="firmado"){
        $solicitudTicket = $this->ticket->whereIn('id',$ids)->update(['firmado'=>$body['status']]);
        $count = $this->ticket->where("firmado", 1)->count();
        $mensaje="Solicitudes firmadas";
      }

      if($body["type"]=="por_firmar"){
        $solicitudTicket = $this->ticket->whereIn('id',$ids)->update(['por_firmar'=>$body['status']]);
